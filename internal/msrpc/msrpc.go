...
var (
	SrvsvcUuid = []byte("c84f324b7016d30112785a47bf6ee188")
	NdrUuid    = []byte("045d888aeb1cc9119fe808002b104860")
)
...
func (r *Bind) Encode(b []byte) {
	...
	hex.Decode(b[32:48], SrvsvcUuid)
	le.PutUint16(b[48:50], SrvsvcVersion)
	le.PutUint16(b[50:52], SrvsvcVersionMinor)

	hex.Decode(b[52:68], NdrUuid)
	le.PutUint32(b[68:72], NdrVersion)
}
...
func (c BindAckDecoder) IsInvalid() bool {
	...
	if c.Version() != RpcVersion {
		return true
	}
	if c.VersionMinor() != RpcVersionMinor {
		return true
	}
	if c.PacketType() != RpcTypeBindAck {
		return true
	}
	return false
}
...
func (r *NetShareEnumAllRequest) Encode(b []byte) {
	...
	le.PutUint16(b[22:24], OpNetShareEnum) // opnum
...
func (c NetShareEnumAllResponseDecoder) IsInvalid() bool {
	...
	if c.PacketType() != RpcTypeResponse {
		return true
	}
